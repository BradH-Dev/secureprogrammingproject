...
#This function will take the file path as input and send it via an HTTP POST request.
def upload_file(file_path, server_url):
    try:
        # Open the file in binary mode
        with open(file_path, 'rb') as file:
            # Define the target URL for the upload endpoint
            url = f"{server_url}/api/upload"
            # Use requests to send a POST request with the file
            files = {'file': file}
...
#This function will allow the client to download a file using the URL provided after the upload.
...
#Here’s how you can modify the send function to handle file upload:
def send(self):
    message = self.msg_entry.get()
    ...
    if message.startswith("upload:"):
        ...
        file_url = upload_file(file_path, "http://127.0.0.1:5000")  # Replace with your server URL
        if file_url:
            self.display_message(f"File uploaded. Access it here: {file_url}")
        self.msg_entry.delete(0, tk.END)
        return

    # File download command
    if message.startswith("download:"):
        file_url = message.split(" ", 1)[1]
        download_file(file_url, "downloaded_file.txt")  # Specify the save path for the file
        self.msg_entry.delete(0, tk.END)
        return

    # Handle other existing chat commands...
